...
export default function PositionsPage() {
  ...
  const onCreate = async (values: CreatePositionInput) => {
    try {
      ...
      setOpen(false); reset(); push('Position created');
  ...
  return (
    <RequireAuth>
      <div className="max-w-3xl">
        <div className="flex items-center justify-between mb-5">
          ...
          {can('positions', 'create') && (
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent title="Create Position">
        ...
        {data && data.data.length === 0 && <EmptyState title="No positions yet" />}
        {data && data.data.length > 0 && (
          <div className="rounded-lg border border-input bg-white overflow-hidden">
            {data.data.map((p) => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 border-t first:border-t-0 border-input text-sm">
                <span className={`flex-1 ${p.is_archived ? 'text-muted-foreground line-through' : ''}`}>{p.title}</span>
