  ...
  return (
    <RequireAuth>
      <div className="max-w-2xl">
        ...
        <div className="text-xs text-muted-foreground mb-4">{data?.unread_count ?? 0} unread</div>
        <div className="flex gap-2 mb-4">
          {(['all', 'unread'] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={cn('text-xs font-medium rounded-md border border-input px-3 py-1.5', filter === f && 'bg-primary text-white border-primary')}>{f}</button>
          ))}
        </div>
        ...
        {data && data.data.length > 0 && (
          <div className="rounded-lg border border-input bg-white overflow-hidden">
            {data.data.map((n: any) => (
              <div key={n.id} className={cn('flex items-start gap-3 px-4 py-3 border-t first:border-t-0 border-input text-sm', !n.is_read && 'bg-accent/5')}>
                <span className={cn('h-1.5 w-1.5 rounded-full mt-1.5 flex-none', n.is_read ? 'bg-transparent' : 'bg-accent')} />
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => { if (n.task_id) router.push(`/tasks/${n.task_id}`); markRead(n.id, true); }}>
                  <div>{n.message}</div>
                  <div className="text-[11px] text-muted-foreground mt-0.5">{formatDate(n.created_at)}</div>
                </div>
                <button onClick={() => markRead(n.id, !n.is_read)} className="text-[11px] text-accent flex-none">{n.is_read ? 'Unread' : 'Read'}</button>
                <button onClick={() => remove(n.id)} className="text-[11px] text-destructive flex-none">Delete</button>
              </div>
            ))}
          </div>
        )}
